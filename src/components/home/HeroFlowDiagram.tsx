...
export const HeroFlowDiagram = () => {
  ...
  // Seta para a direita (Linha de cima)
  const ArrowR = () => (
  ...
  // Seta para a esquerda (Linha de baixo)
  const ArrowL = () => (
    <div className="hidden md:flex items-center px-2 text-slate-200">
  ...
  return (
    <div className="relative p-8 bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/60 w-full max-w-2xl z-10">
      {/* === SVG DA CURVA LATERAL (Conecta 3 ao 4) === */}
      {/* Agora a curva fica apenas no lado direito, conectando o item de cima com o de baixo */}
      <svg
      ...
      >
        ...
        {/* Curva em "C" na direita: Sai do item 3 e entra no item 4 */}
        <path
          d="M 540 80 C 600 80, 600 240, 540 240"
        ...
        {/* Ponta da Seta (Apontando para a esquerda entrando no 4) */}
        <path d="M 545 240 L 555 230 M 545 240 L 555 250" stroke="#22c55e" strokeWidth="4" strokeLinecap="round" />
      </svg>
      ...
      <div className="flex flex-col gap-12 relative z-10">
        ...
        <div className="flex items-start justify-between px-2">
          ...
          <ArrowR />
          {/* O último item da linha 1 alinha com a curva */}
          <StepItem step={steps[2]} />
        </div>

        {/* === LINHA 2 INVERTIDA (6 <- 5 <- 4) === */}
        {/* Renderizamos: Item 6 (Esq), Seta, Item 5 (Meio), Seta, Item 4 (Dir) */}
        {/* Isso coloca o Item 4 exatamente embaixo do Item 3 */}
        <div className="flex items-start justify-between px-2">
          <StepItem step={steps[5]} /> {/* Item 6 */}
          <ArrowL />
          <StepItem step={steps[4]} /> {/* Item 5 */}
          <ArrowL />
          <StepItem step={steps[3]} /> {/* Item 4 */}
        </div>
      </div>
    </div>
  );
